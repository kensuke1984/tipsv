...
! --- inputting parameter ---
    ...
    if( (rmax-r0)<shallowdepth) then ! option for shallow events
        ...
        call calra(inlayer,jnlayer,jnslay,jnllay,dzpar,nzone,vrmin,vrmax,iphase,&
        ...
        call calsp(ndc,nsl,nll,iphase,nlayer,nllay,isp,jsp,ksp,issp,ilsp,lsp,jssp,isdr,jsdr,ildr,jdr,kdr )
        ...
        rhoinv(1:vnp)   = 1 / rho(1:vnp)
        kappainv(1:vnp) = 1 / kappa(1:vnp)
        isl = 0
        ill = 0
        do i=1,ndc+1
            if ( iphase(i)==1 ) then
...
! computing the structure grid points
    ...
    do i=1,ndc+1
        if ( iphase(i)==1 ) then
            ...
            call calmatc( nlayer(i),vnp,vra,rho ,2,0,0,ra(isp(i)), t(itmp) )
            call caltl( nlayer(i),vnp,vra,rho,ra(isp(i)),work(itmp) )
            t(itmp:itmp+4*nlayer(i)-1)=(t(itmp:itmp+4*nlayer(i)-1)+work(itmp:itmp+4*nlayer(i)-1))/2
            call calmatc( nlayer(i),vnp,vra,ecKx,0,0,0,ra(isp(i)),h1x(itmp) )
            call calhl( nlayer(i),vnp,vra,ecKx,ra(isp(i)),work(itmp) )
            h1x(itmp:itmp+4*nlayer(i)-1)=(work(itmp:itmp+4*nlayer(i)-1)+h1x(itmp:itmp+4*nlayer(i)-1))/2
            call calmatc( nlayer(i),vnp,vra,ecKy,0,0,0,ra(isp(i)),h1y(itmp) )
...
!      call simplesplit (imin, imax, PETOT, mpimin, mpimax)
        ...
        if ( i/=0 ) then
            ...
            do j=1,ndc+1
                if ( iphase(j)==1 ) then
                    ...
                    jtmp = jdr+jsp(j)
                    mtmp = kdr+ksp(j)
                    call cala0( nlayer(j),omega,omegai, t(itmp),h1x(itmp),h2L(itmp),h2N(itmp),&
                        h3ay(itmp),h4aL(itmp),h4aN(itmp),h5ay(itmp),h6aL(itmp),h6aN(itmp),&
                        h7y(itmp),h7z(itmp),h8L(itmp), h8N(itmp),coef1(j),coef2(j),cwork(jtmp) )
                    call overlapa( nlayer(j),cwork(jtmp),a0(1,mtmp) )
                    call cala1( nlayer(j),h1x(itmp),h2L(itmp),h2N(itmp),h3y(itmp),h4L(itmp),h4N(itmp), &
                        h5y(itmp),h6L(itmp), h6N(itmp),coef1(j),coef2(j),cwork(jtmp) )
                    call overlapa( nlayer(j),cwork(jtmp),a1(1,mtmp) )
                    call cala2( nlayer(j),h1x(itmp),h2L(itmp),h2N(itmp),coef1(j),coef2(j),cwork(jtmp) )
                    call overlapa( nlayer(j),cwork(jtmp),a2(1,mtmp) )
                    jtmp = jsdr+jssp(isl)
                    call calhml( nlayer(j),coef1(j),coef2(j),h3my(-2,jtmp),&
                        h5my(-1,jtmp), h4m1L(-1,jtmp),h4m2N(-2,jtmp),h6m1N(-1,jtmp), h6m2L(-2,jtmp),a1(1,mtmp) )
                else
                    ill = ill + 1
                    itmp = ildr+ilsp(ill)
                    jtmp = jdr+jsp(j)
    ...
    ! ************************** Files Handling **************************
        do ir = 1 ,nr
            open(unit=1,file=trim(output(ir)),status='replace', form='unformatted',&
