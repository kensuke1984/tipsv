program tipsv
    ...
    enddo
    if ( (r0.lt.rmin) .or. (r0.gt.rmax) ) stop 'Location of the source is improper.'
    !
    ...
    if( (rmax-r0).lt.shallowdepth) then ! option for shallow events
        ...
        if ( inlayer.gt.maxnlay ) stop 'The number of grid points is too large.'
        if ( nslay.gt.maxnslay ) stop 'The number of the grid points in the solid is too large.'
        if ( nllay.gt.maxnllay ) stop 'The number of the grid points in the liquid is too large.'
        if ( jnlayer.gt.2*maxnlay ) stop 'The number of the total grid points is too large.'
        if ( ( jnslay.gt.2*maxnslay ).or.( jnllay.gt.2*maxnllay ) ) stop 'The number of the total grid points is too large.'
    ...
    if ( nslay.gt.maxnslay )&
        stop 'The number of the grid points in the solid is too large.'
    if ( nllay.gt.maxnllay )&
        stop 'The number of the grid points in the liquid is too large.'
    if ( jnlayer.gt.2*maxnlay )&
        stop 'The number of the total grid points is too large.'
                    ...
                    if ( iabs(m).le.iabs(l) ) then
                        ...
                        if ( l.eq.0 ) then
                            ...
                            if( mod(l,100).eq.0) then
                                if ( ( m.eq.-2 ).or.( m.eq.-l ) ) then
                                    call dcsymbdl0( a(1,itmp),3,nn-itmp+1,&
                                        6,eps, z(itmp),w(itmp),ll,lli,llj,ier )
                                endif
                                call dcsbdlv0( a(1,itmp),g(itmp),3,&
                                    nn-itmp+1,eps,z(itmp),ier )
                                do jj=1,nn ! sum up c of the same l
