...
c     &        tlen,np,omegai,imin,imax,nnlayer,
c     &        nzone,vrmin,vrmax,rrho,
c     &        vpv,vph,vsv,vsh,eta,qmu,qkappa,
c     &        r0,eqlat,eqlon,mt,nr,theta,phi,lat,lon,output)
...
c computing the excitation vector
                        call cvecinit( nn,g )
c                        call calg( l,m,coef1(spn),coef2(spn),lsq,
c     &                       ecC0,ecF0,ecL0,ya,yb,yc,yd,
c     &                       ra(mtmp),r0,mt,g(jtmp) )
                      if(m.eq.0) then
                           g(nn-1)=-dconjg(rdvec(1,0))*singleforce(1)
                        else
                           g(nn-1)=-(dconjg(rdvec(2,m))*singleforce(2)+
     &                          dconjg(rdvec(3,m))*singleforce(3))
     &                          /dcmplx(lsq)
                        endif
